...
{
	fprintf(stderr, "usage: tmate-ssh-server [-b ip] [-h hostname] [-k keys_dir] [-a authorized_keys_path] [-p listen_port] [-q ssh_port_advertized] [-w websocket_hostname] [-z websocket_port] [-x] [-v]\n");
}
...
int main(int argc, char **argv, char **envp)
{
	...
	while ((opt = getopt(argc, argv, "b:h:k:a:p:q:w:z:xv")) != -1) {
		switch (opt) {
		case 'b':
